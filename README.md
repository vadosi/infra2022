...
- Nginx - легкий опенсорсный веб-сервер, позволяющий хостить веб-сайты, балансировать нагрузку, поднять почтовый сервер и проксировать соединения дальше. Завоевал 20% рынка в живом вебе, уступает по доле лишь Apache, в РФ используется на 2/3 всех сайтов. Под капотом nginx в конфиге устанавливается порт (80/443 обычно), который он слушает и исполняемый код (в нашем ДЗ - JBrowser), с которым взаимодействует клиент. Процессы выглядят примерно так:
![image](https://user-images.githubusercontent.com/58905528/209319013-4fe6a309-1074-4459-a067-8acd7397e6c2.png)
Такая параллелизация помогает обработать до 1024 запросов одномоментно, но для масштабирования на помощь приходят модули. Среди конкурентов уже упомянутый Apche HTTP Server, Cherokee HTTP Server и питоновское чудо Gunicorn 


[0.4] What is SSH, and for what is it typically used? Explain two ways to authenticate in an SSH server in detail.

- SSH - безопасный протокол, типично использующий 22 порт, туннелирующий соединение, что позволяет использовать его для удаленного управления серверами. Вики утверждает о возможности заворачивания в SSH любого другого протокола для повышения безопасности, доступны различные алгоритмы шифорвания. Аутентификация происходит двумя способами:
1. Генерятся RSA-ключики, публичный отдаем серверу (по дефолту лежат стопкой в ~/.ssh/authorized_keys), с помощью приватного подключаемся к нему. 
2. Банальный логин-пароль. Я такого не видел, видимо потому что SSH не имеет смысла в сочетании с 123456. При каждом подключении как в HTTPS вырабатывается общий секретный ключ для шифрования трафика.
